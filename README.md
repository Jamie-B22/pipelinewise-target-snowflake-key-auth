...
- install with `meltano add --custom loader target-snowflake-key-auth`
  ...
  - settings: `account,dbname,user,warehouse,file_format,role,default_target_schema,aws_region_name,aws_profile,snowflake_private_key_aws_parameter_name,snowflake_private_key_code_aws_parameter_name,data_flattening_max_level:integer,add_metadata_columns:boolean`
